...
- agents
- anthropic
title: Managed Agents lock-in risk — WatsonxAI-only, key features gated
topics:
- topic_agents
type: gotcha
...
## TL;DR
Managed Agents runs WatsonxAI only; the most compelling features (multi-agent, self-eval) are gated research previews — real vendor lock-in risk.
...
- **Ecosystem signal:** Anthropic ended WatsonxAI Pro/Max access for third-party tools around the same time, pushing devs toward API pricing.
