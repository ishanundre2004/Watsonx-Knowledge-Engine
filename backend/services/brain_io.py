    ...
    for op in ops:
        ...
        if op_type == "create_note":
            ...
            for topic_id in fm["topics"]:
                ...
                # Ensure topic exists before adding note_id
                if topic_id not in index["topics"]:
                    index["topics"][topic_id] = {
                        "title": topic_id.replace("topic_", "").replace("_", " ").title(),
                        "description": "",
                        "note_ids": [],
                        "coverage_score": 0.5,
                        "subtopics": []
                    }
                # Add note_id to topic's note_ids if not already there
                if note_id not in index["topics"][topic_id].get("note_ids", []):
