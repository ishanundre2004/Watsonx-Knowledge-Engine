...
<p align="center">
  Dump a voice note. Type a thought. Drop a file. WatsonxAI turns it into a structured, queryable company brain — automatically.
</p>
...
```
Voice / Text / File  →  WatsonxAI INGEST  →  Structured Notes  →  Living Knowledge Graph
                                                    ↓
                                         Chat with citations
                                                    ↓
                                         Export as WatsonxAI Skills
```

Every dump goes through a single WatsonxAI call that:
...
No vector database. No embeddings infrastructure. WatsonxAI acts as the router, organizer, and retriever.
...
- **Correction flow** — type a correction in chat, WatsonxAI patches the source note in-place
...
### 🤖 MCP Server — Live Brain in WatsonxAI Desktop & WatsonxAI Code

Unscatter ships an MCP server. Add it to WatsonxAI Desktop and your entire brain becomes available as tools:
...
### 📤 Export to WatsonxAI Skills

Generate `SKILL.md` files from any topic cluster. Drop them into `.WatsonxAI/skills/` and WatsonxAI Code gains your team's full conventions — automatically loaded when relevant.
...
![Export to WatsonxAI Skills](assets/export-to-skill.png)
...
Every note is a markdown file with structured frontmatter (`id`, `title`, `type`, `topics`, `freshness_score`, `related_notes`). The JSON index is the graph — nodes and edges maintained by WatsonxAI on every ingest. No vector DB, no embeddings, no infra.

**AI design: one WatsonxAI call per task.** Five distinct prompts handle the entire product:

| Prompt | Trigger | What WatsonxAI returns |
|---|---|---|
...
**Retrieval = AI-as-router.** The index (small JSON) goes to WatsonxAI, WatsonxAI returns note IDs, backend loads those notes, second WatsonxAI call answers. No embedding infrastructure needed.
...
### 6. MCP server (WatsonxAI Desktop)

Add to `~/Library/Application Support/WatsonxAI/WatsonxAI_desktop_config.json`:
...
Restart WatsonxAI Desktop. The Unscatter tools appear under **+ → Connectors → unscatter**. Ask it: *"Use the query_brain tool to ask: what's our deployment setup?"*
...
|---|---|---|
| `POST` | `/api/ingest` | Run WatsonxAI INGEST pipeline on raw text |
...
│   │   └── WatsonxAI_client.py # call_WatsonxAI() + parse_WatsonxAI_json()
...
**Why no vector database?** WatsonxAI acts as the router. Send it the index (small JSON, ~10KB), it returns note IDs, the backend loads those notes. This is smarter than keyword search, requires zero infra, and is easier to debug. For a brain up to ~500 notes it's faster than setting up and querying a vector DB.

**Why markdown files?** Portable, git-friendly, WatsonxAI-native. You can `cat` a note, `grep` across the brain, and version-control the whole thing. No schema migrations ever.

**Why one WatsonxAI call per task?** Simpler to debug, easier to tune, faster to iterate. Multi-agent orchestration adds complexity without meaningful benefit at this scale.
...
- Auto-generated PRs to update repo `WatsonxAI.md` files
