...
from backend.services.WatsonxAI_client import call_WatsonxAI, parse_WatsonxAI_json
...
def query_brain(question: str) -> str:
    ...
    index = load_index()
    router_result = parse_WatsonxAI_json(call_WatsonxAI(
    ...
    answer = call_WatsonxAI(CHAT_ANSWER_PROMPT.format(
        question=question,
...
def capture(content: str, modality: str = "text") -> str:
    """Add new knowledge to the company brain. WatsonxAI will structure and link it automatically."""
    index = load_index()
    ...
    result = parse_WatsonxAI_json(call_WatsonxAI(prompt))
