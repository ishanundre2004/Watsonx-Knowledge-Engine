    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="flex max-h-[85vh] max-w-3xl flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-200">
        <DialogHeader className="shrink-0">
          <DialogTitle>Export to WatsonxAI Skills</DialogTitle>
          <DialogDescription>
            Generate SKILL.md files WatsonxAI Code loads automatically — giving your AI tools full context about your team's conventions.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="flex min-h-0 flex-1 overflow-hidden rounded-lg border border-border/60 bg-zinc-950">
              {loading ? (
                <div className="flex flex-1 flex-col items-center justify-center gap-4 text-muted-foreground">
                  ...
                  <div className="text-sm">WatsonxAI is writing your skill{selected.size !== 1 ? "s" : ""}…</div>
                ...
                <div className="flex flex-1 flex-col items-center justify-center gap-3 text-muted-foreground">
                  ...
                  <div className="max-w-xs text-center text-xs">
                    Each SKILL.md tells WatsonxAI exactly when to load it and what conventions to follow.
                  </div>
                </div>
              )}
            </div>
            ...
            {skills.length > 0 && (
              <div className="shrink-0 space-y-2 rounded-lg border border-primary/20 bg-primary/5 p-4 text-xs">
                <div className="text-sm font-semibold text-foreground">
                  How to wire this into WatsonxAI Code
                </div>
                <div className="space-y-1 text-muted-foreground">
                  <p>1. Download and drop into your repo's <code className="text-primary">.WatsonxAI/skills/</code> directory.</p>
                  <p>2. WatsonxAI Code auto-discovers skills — no config needed.</p>
                  <p>3. When WatsonxAI touches relevant files, it loads the skill and applies your team's conventions.</p>
                </div>
              </div>
            )}
          </div>
